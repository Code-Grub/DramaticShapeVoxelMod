...
local default = Billboards.mesh({
  image = "large-sheet.png", frameWidth = 32, frameHeight = 32,
}, 1)
eq(default.verts[1][1], -8, "a 32px default-anchored card starts left of its tile")
eq(default.verts[2][1], 24, "a 32px default-anchored card keeps its full width")
eq(default.verts[1][2], 0, "the default bottom anchor remains grounded")
eq(default.verts[3][2], 32, "the default card keeps its full height")
eq(default.verts[1][5], (64 - 0.05) / 96,
...
eq(#made, 2, "anchor metadata separates otherwise identical cached cards")
