...
- [Stadium Battle FX](https://github.com/anxiousintrovert/StadiumBattleFX) discovers five API-1 providers from Battle Art: `BATTLE ART VOXEL MAP` (arena), `BATTLE ART NATIVE CARDS` (models), `BATTLE ART PLACED CAMERA` (camera), `BATTLE ART PROJECTED HUD` (HUD), and `BATTLE ART EXIT FADE` (transition). With SBFX installed, select `BATTLE ART VOXEL MAP` in **BTL ARENA** to enable Battle Art's battle features; SBFX then owns the complete battle lifecycle and its animations, effects, announcer, overlay, and all other unselected slots remain independently selectable. `BATTLE ART PROJECTED HUD` renders Battle Art's HUD bands in the arena and continues to use Battle Art's `HUD SCALE` setting. Without SBFX, Battle Art's normal standalone battle renderer is unchanged. Battle Art's native cards are only handed to the SBFX arena when the model provider is explicitly selected, so `OFF` and SBFX's own model providers remain meaningful choices. The native-card provider also supplies projected centers, footprints, attachment tags, and a BACK SPRITES camera lock, with explicit invalidation on graphics reset.
