"""Import Pokemon Yellow JP GBC front sprites as the Gen 1 shiny set.

The Gen 1 animated collection uses ordinary one-frame PNGs, so no atlas or
timing sidecar is generated. Bulbagarden files are preserved byte-for-byte and
renamed only to the mod's battle-art species slugs.
"""
...
def main() -> None:
    ...
    destination = root / "assets/battle/front-animated/gen1/shiny"
    destination.mkdir(parents=True, exist_ok=True)
    roster = species(root)
    seen: set[str] = set()

    for number, (_, slug) in enumerate(roster, 1):
        archive_name = f"Spr 1y {number:03d} GBC JP.png"
